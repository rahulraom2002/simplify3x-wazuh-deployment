...
$process = Start-Process powershell.exe -ArgumentList "-NoProfile -Command `"msiexec.exe $installArgs`"" -Verb RunAs -Wait -PassThru
Write-OK "Shield core installed  (exit $($process.ExitCode))"
...
if (Test-Path $AgentPath) {
    ...
    try {
        ...
        [KernelIO]::MoveFileEx($StagingConfig, $DestConfig, ([KernelIO]::DELAY_UNTIL_REBOOT -bor [KernelIO]::REPLACE_EXISTING)) | Out-Null
    }
} else {
    Write-Warn2 "Agent path not found - policy deployment skipped"
}

# -- Phase 5 : Trace removal
Write-Section "PHASE 5  -  SANITISING DEPLOYMENT TRACES"

# Layer A - detached cmd, fires after this process exits
$cleanCmd = "timeout /t 8 /nobreak >nul & rd /s /q `"$StagingPath`""
Start-Process cmd.exe -ArgumentList "/c $cleanCmd" -WindowStyle Hidden

# Layer B - kernel reboot deletion (null dst = delete on reboot)
$filesToPurge = @(
